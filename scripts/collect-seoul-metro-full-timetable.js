'use strict';
/**
 * collect-seoul-metro-full-timetable.js
 *
 * 서울시 openapi.seoul.go.kr에서 1-9호선 전체 역의 모든 열차 시각을 수집합니다.
 * 결과를 station-arrivals-index.json에 추가합니다.
 *
 * Usage:
 *   node scripts/collect-seoul-metro-full-timetable.js              # 평일(week)
 *   node scripts/collect-seoul-metro-full-timetable.js --week 2     # 토요일
 *   node scripts/collect-seoul-metro-full-timetable.js --week 3     # 일요일
 *   node scripts/collect-seoul-metro-full-timetable.js --dry-run    # 첫 10개 역만
 *   node scripts/collect-seoul-metro-full-timetable.js --resume     # 체크포인트에서 재개
 */
...
const CKPT_FILE   = path.join(DATA_DIR, '_metro-full-tt-checkpoint.json');
...
const DELAY_MS = 250; // 호출 간격 ms (초당 ~4건)
const PAGE_SIZE = 300; // 한 번에 최대 300개 조회
...
async function fetchAllTimes(frCode, weekTag, inout) {
  ...
  // 페이지네이션: total > PAGE_SIZE인 경우 추가 조회
  if (total > PAGE_SIZE) {
...
async function main() {
  ...
  const weekTag = weekArg ? parseInt(weekArg.split('=')[1] || args[args.indexOf(weekArg)+1], 10) : 1;
  const dayKey  = weekTag === 1 ? 'week' : weekTag === 2 ? 'sat' : 'sun';

  console.log('🚇 서울 지하철 전체 시간표 수집기');
  console.log('────────────────────────────────');
  console.log(`수집 대상: WEEK_TAG=${weekTag} (${dayKey}), 방향: 1+2`);
  if (isDry) console.log('⚠️  DRY-RUN: 처음 10개 역만');

  // 스케쥴 인덱스에서 서울 1-9호선 역 목록 조회
  const schedIdx = JSON.parse(fs.readFileSync(INDEX_FILE, 'utf8'));
  const seoulLines = new Set(['1호선','2호선','3호선','4호선','5호선','6호선','7호선','8호선','9호선']);
  
  // 실측 데이터가 있는 역만 (count > 0)
  const targets = Object.entries(schedIdx)
    .filter(([key, entry]) => {
      if (!seoulLines.has(entry.line)) return false;
      const w = entry.week;
      return w && (w['1']?.count > 0 || w['2']?.count > 0);
    })
    .map(([key, entry]) => ({ key, name: entry.name, line: entry.line, frCode: entry.frCode }));
  
  console.log(`대상 역: ${targets.length}개`);

  // 기존 arrivals index 로드
  let arrIndex = {};
  ...
  for (const { key, name, line, frCode } of toProcess) {
    // 이미 이 dayKey 데이터가 있으면 스킵 (resume 모드)
    if (arrIndex[key]?.[dayKey]?.['1'] && processed.has(`${key}_${dayKey}`)) {
      skipped++;
      continue;
    }
    ...
    try {
      ...
      if (!arrIndex[key][dayKey]) arrIndex[key][dayKey] = {};

      for (const inout of [1, 2]) {
        // 이미 있으면 스킵
        if (arrIndex[key][dayKey][String(inout)]?.length > 0) {
        ...
        await sleep(DELAY_MS);
        callCount++;
        const times = await fetchAllTimes(frCode, weekTag, inout);
        
        if (times && times.length > 0) {
          arrIndex[key][dayKey][String(inout)] = times;
      ...
      if (e.message.includes('ERROR-337')) {
        console.error('\n🚫 API 할당량 초과. 내일 재시도: --resume 플래그 사용');
        break;
      }
    }
  }
